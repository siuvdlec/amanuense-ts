export function toValidVariableName(input: string, casing: 'camel' | 'pascal'): string {
    const joined = input
        .split(' ')
...
export function capitalize(input: string, casing: 'camel' | 'pascal'): string {
    const firstChar = casing === 'camel' ? input.charAt(0).toLocaleLowerCase() : input.charAt(0).toLocaleUpperCase()
