    ...
    return pipe(
        ...
        RTE.chain(components =>
            pipe(
                ...
                RTE.chain(() => generateParameters(Object.values(components.parameters))),
                RTE.chain(() => generateResponses(Object.values(components.responses))),
                RTE.chain(() => generateRequestBodies(Object.values(components.requestBodies)))
            )
        )
    )
}

function generateSchemas(schemas: ParsedItem<TypeDeclaration>[]): CodegenRTE<void> {
    return writeComponentsFiles(SCHEMAS_PATH, schemas, writeSchemaFile)
}

function writeSchemaFile(declaration: ParsedItem<TypeDeclaration>): CodegenRTE<void> {
...
function getSchemaFileImports(declaration: ParsedItem<TypeDeclaration>): string {
...
function generateParameters(parameters: ParsedItem<ParsedParameter>[]): CodegenRTE<void> {
    return writeComponentsFiles(PARAMETERS_PATH, parameters, writeParameterFile)
}

function writeParameterFile(parameter: ParsedItem<ParsedParameter>): CodegenRTE<void> {
    const content = `${getImports()}
    import { OperationParameter } from "${RUNTIME_PACKAGE}";
    
    ${generateSchemaIfDeclaration(parameter.item.type)}
    
    export const ${parameter.name}: OperationParameter = ${generateOperationParameter(parameter.item)}`

    return writeGeneratedFile(PARAMETERS_PATH, `${parameter.name}.ts`, content)
}

function generateResponses(responses: ParsedItem<ParsedResponse>[]): CodegenRTE<void> {
    return writeComponentsFiles(RESPONSES_PATH, responses, writeResponseFile)
}

function writeResponseFile(response: ParsedItem<ParsedResponse>): CodegenRTE<void> {
    return pipe(
        generateOperationResponse(response),
        RTE.map(def => `export const ${response.name} = ${def};`),
        RTE.chain(content => writeGeneratedFile(RESPONSES_PATH, `${response.name}.ts`, content))
    )
}

function generateRequestBodies(bodies: ParsedItem<ParsedBody>[]): CodegenRTE<void> {
    return writeComponentsFiles(REQUEST_BODIES_PATH, bodies, writeRequestBodyFile)
}

function writeRequestBodyFile(parsedBody: ParsedItem<ParsedBody>): CodegenRTE<void> {
    return pipe(
        RTE.Do,
        RTE.bind('schema', () => RTE.right(generateOperationBodySchema(parsedBody.item))),
      ...
      export const ${parsedBody.name}: OperationBody = ${body}`
        ...
        RTE.chain(content => writeGeneratedFile(REQUEST_BODIES_PATH, `${parsedBody.name}.ts`, content))
    )
}
