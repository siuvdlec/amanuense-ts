...
import { FORM_ENCODED_MEDIA_TYPE, JSON_MEDIA_TYPE, TEXT_PLAIN_MEDIA_TYPE } from '../../definition'
import type { BodyItemOrRef } from '../parser/body'
import type { ParsedItem } from '../parser/common'
import type { ParsedOperation } from '../parser/operation'
import type { ParameterItemOrRef } from '../parser/parameter'
import type { ParsedJsonResponse, ResponseItemOrRef } from '../parser/response'
import { capitalize } from '../utils'
import { generateOperationBody, generateOperationBodySchema, getBodyOrRefStaticType } from './body'
import {
...
export function generateOperations(): CodegenRTE<void> {
    return pipe(
        ...
        RTE.chain(operations => R.traverseWithIndex(RTE.ApplicativeSeq)(generateOperation)(operations)),
        RTE.map(() => void 0)
    )
}
...
function generateOperation(operationId: string, operation: ParsedOperation): CodegenRTE<void> {
    return pipe(
        generateItems(operationId, operation),
        RTE.chain(items => generateFileContent(operationId, operation, items)),
        RTE.chain(content => writeGeneratedFile(OPERATIONS_PATH, `${operationId}.ts`, content))
    )
}

function generateItems(operationId: string, operation: ParsedOperation): CodegenRTE<GeneratedItems> {
    return pipe(
        RTE.Do,
        RTE.bind('parameters', () => generateOperationParameters(operationId, operation.parameters)),
...
function generateFileContent(
  ...
  import { Operation } from "${RUNTIME_PACKAGE}";
...
function generateOperationParameters(
    ...
    return pipe(
        ...
        RTE.bind('definition', () => generateOperationParametersDefinition(parameters)),
        RTE.bind('requestMap', () => generateRequestParametersMap(operationId, parameters))
    )
}

function generateOperationParametersSchemas(parameters: ParameterItemOrRef[]): CodegenRTE<string> {
    const schemas = pipe(
        parameters,
...
function generateOperationParametersDefinition(parameters: ParameterItemOrRef[]): CodegenRTE<string> {
    return pipe(
        parameters,
...
function generateOperationParameterDefinition(parameter: ParameterItemOrRef): CodegenRTE<string> {
    return pipe(
...
function generateRequestParametersMap(operationId: string, parameters: ParameterItemOrRef[]): CodegenRTE<string> {
...
function generateRequestParameter(itemOrRef: ParameterItemOrRef): CodegenRTE<string> {
    return pipe(
...
function generateBody(itemOrRef: O.Option<BodyItemOrRef>): CodegenRTE<GeneratedBody | undefined> {
    ...
    return pipe(
        ...
        RTE.map(({ body, requestBody }) => {
            const res: GeneratedBody = {
                ...
                requestBody,
                schema: isParsedItem(itemOrRef.value) ? generateOperationBodySchema(itemOrRef.value.item) : undefined,
            }
...
function generateResponses(responses: Record<string, ResponseItemOrRef>): CodegenRTE<GeneratedResponses> {
    return pipe(
        RTE.Do,
...
function generateResponsesSchemas(responses: Record<string, ResponseItemOrRef>): CodegenRTE<string> {
    return RTE.right(
...
function isParsedJsonResponse(response: ResponseItemOrRef): response is ParsedItem<ParsedJsonResponse> {
    return isParsedItem(response) && response.item._tag === 'ParsedJsonResponse'
}

function getReturnType(responses: Record<string, ResponseItemOrRef>): CodegenRTE<string> {
    ...
    return pipe(
        ...
        RTE.map(response => {
            ...
            const staticType =
                type.kind === 'TypeDeclaration' ? `${getItemOrRefPrefix(response)}${type.name}` : gen.printStatic(type)
...
function getContentTypeHeader(operation: ParsedOperation): CodegenRTE<string | undefined> {
    return pipe(
        operation.body,
        O.fold(
            () => RTE.right(undefined),
...
function getAcceptHeader(operation: ParsedOperation): CodegenRTE<string | undefined> {
    ...
    return pipe(
        getParsedItem(successfulResponse),
        RTE.map(response => (response.item._tag === 'ParsedJsonResponse' ? JSON_MEDIA_TYPE : undefined))
    )
}

function getSuccessfulResponse(responses: Record<string, ResponseItemOrRef>): ResponseItemOrRef | undefined {
