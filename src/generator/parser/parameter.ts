...
import { OperationParameterIn, JSON_MEDIA_TYPE } from '../../definition'
import { JsonReference } from './JSONReference'
import { ComponentRef, createComponentRef, getOrCreateType, parsedItem, ParsedItem } from './common'
import type { ParserContext, ParserRTE } from './context'
...
export type ParameterItemOrRef = ParsedItem<ParsedParameter> | ComponentRef<'parameters'>
...
export function parseParameterObject(
    ...
    if (param.schema !== undefined) {
        return parseParameterWithSchema(name, param, param.schema, 'ParsedFormParameter')
    }

    const jsonContentSchema = param.content?.[JSON_MEDIA_TYPE].schema

    if (jsonContentSchema !== undefined) {
        return parseParameterWithSchema(name, param, jsonContentSchema, 'ParsedJsonParameter')
    }

    return RTE.left(new Error(`Error parsing parameter ${param.name}: no schema or application/json schema defined`))
}
...
function parseParameterWithSchema(
    ...
    return pipe(
        ...
        RTE.map(({ type, defaultValue }) => buildParsedParameter(param, type, defaultValue, tag))
    )
}

function getDefaultValue(s: OpenAPIV3.ReferenceObject | OpenAPIV3.SchemaObject): ParserRTE<unknown> {
    return pipe(
        RTE.Do,
        RTE.bind('schema', () =>
            JsonReference.is(s)
...
function getOpenapiSchemaFromRef(ref: OpenAPIV3.ReferenceObject): ParserRTE<OpenAPIV3.SchemaObject> {
    return pipe(RTE.asks(context => context.document.components?.schemas?.[ref.$ref] as OpenAPIV3.SchemaObject))
}
