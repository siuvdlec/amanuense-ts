...
function parseJsonReference(pointer: string): E.Either<Error, gen.TypeReference> {
    return pipe(
        createJsonPointer(pointer),
        E.chain(jsonPointer => checkValidReference('schemas', jsonPointer)),
        E.map(jsonPointer => {
            const name = `${jsonPointer.tokens[2]}.${toValidVariableName(jsonPointer.tokens[3] as string, 'pascal')}`
...
function parseAllOf(
    ...
    return pipe(
        ...
        E.map(ss => gen.intersectionCombinator(ss))
    )
}
...
function parseOneOf(
    ...
    return pipe(
        parseSchemas(schemas),
        E.map(ss => gen.unionCombinator(ss))
    )
}

function parseString(schema: OpenAPIV3.SchemaObject): E.Either<Error, gen.TypeReference> {
    if (schema.enum) {
        return parseEnum(schema.enum)
    }

    if (schema.format === 'date' || schema.format === 'date-time') {
        return E.right(gen.customCombinator('Date', 'DateFromISOString', ['DateFromISOString']))
    }
...
function parseArray(schema: OpenAPIV3.ArraySchemaObject): E.Either<Error, gen.TypeReference> {
    return pipe(
...
function parseSchemas(
    ...
    return pipe(schemas, E.traverseArray(parseSchema)) as E.Either<Error, gen.TypeReference[]>
}

function parseObject(schema: OpenAPIV3.NonArraySchemaObject): E.Either<Error, gen.TypeReference> {
    if (schema.properties) {
        return pipe(
            Object.entries(schema.properties),
            E.traverseArray(([name, propSchema]) => parseProperty(name, propSchema, schema)),
...
function parseProperty(
    ...
    return pipe(
        parseSchema(schema),
        E.map(t => gen.property(name, t, parentSchema.required ? !parentSchema.required.includes(name) : true))
    )
}
