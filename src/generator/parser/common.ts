...
import type { ParsedBody } from './body'
import { ParserRTE, readParserOutput } from './context'
import type { ParsedParameter } from './parameter'
import type { ParsedResponse } from './response'
...
function componentRef<T extends ComponentType>(componentType: T, pointer: string): ComponentRef<T> {
    return {
        _tag: 'ComponentRef',
        componentType,
        pointer,
...
export type ComponentRefItemType<C extends ComponentType> = ParsedComponents[C][string]

export type ItemOrRef<C extends ComponentType> = ComponentRefItemType<C> | ComponentRef<C>

export function checkValidReference(componentType: ComponentType, pointer: JsonPointer): E.Either<Error, JsonPointer> {
    const { tokens } = pointer

    if (tokens.length === 4 && tokens[1] === 'components' && tokens[2] === componentType) {
    ...
    return pipe(
        ...
        RTE.chain(component =>
            component
                ...
                : RTE.left(
                      new Error(`Cannot get component name for componentType ${componentType}, pointer ${pointer}`)
                  )
        )
    )
}
...
export function getOrCreateType(
    ...
    if (JsonReference.is(schema)) {
        return pipe(
            ...
            RTE.map(component => gen.customCombinator(`schemas.${component.name}`, `schemas.${component.name}`))
        )
    }
...
function createDeclarationOrReturnType(name: string, type: gen.TypeReference): gen.TypeDeclaration | gen.TypeReference {
    return shouldCreateDeclaration(type) ? gen.typeDeclaration(name, type, true) : type
}
