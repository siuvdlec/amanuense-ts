...
export function parseAllComponents(): ParserRTE<void> {
    return pipe(
        ...
        RTE.chain(components => {
            const tasks: ParserRTE<void>[] = components ? createTasks(components) : []
            return pipe(
...
function createTasks(components: OpenAPIV3.ComponentsObject): ParserRTE<void>[] {
...
function parseAllParameters(
    ...
    return pipe(
        ...
        RTE.traverseSeqArray(([name, param]) => parseParameterComponent(pointer, name, param)),
        RTE.map(() => void 0)
    )
}
...
function parseAllBodies(
    componentsPointer: JsonPointer,
    bodies: Record<string, OpenAPIV3.ReferenceObject | OpenAPIV3.RequestBodyObject>
): ParserRTE<void> {
    const pointer = componentsPointer.concat(['requestBodies'])

    return pipe(
        Object.entries(bodies),
        RTE.traverseSeqArray(([name, body]) => parseBodyComponent(pointer, name, body)),
        RTE.map(() => void 0)
    )
}
...
function parseAllResponses(
    componentsPointer: JsonPointer,
    responses: Record<string, OpenAPIV3.ReferenceObject | OpenAPIV3.ResponseObject>
): ParserRTE<void> {
    const pointer = componentsPointer.concat(['responses'])

    return pipe(
        Object.entries(responses),
        RTE.traverseSeqArray(([name, response]) => parseResponseComponent(pointer, name, response)),
        RTE.map(() => void 0)
    )
}
